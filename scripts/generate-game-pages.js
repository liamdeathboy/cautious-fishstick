...
function formatMetaDetails(slug, data, override) {
  ...
  return { genre, platforms, release };
}
...
function buildSchema(slug, data, override, metaDetails, imageUrl) {
  ...
  // NOTE: we intentionally do NOT emit aggregateRating — Schplay has no real
  // user-review data, and marking up fabricated ratings violates Google policy.
  return {
...
function buildStaticGameLinks(currentSlug, gameList) {
  ...
  // 2) Long-tail coverage via a deterministic round-robin window over the full
  //    catalog, rather than taking the top 2 of a fresh per-page shuffle. The
  //    old approach drew each page's long-tail links independently, so with
  //    ~275 long-tail games and only 2 slots per page, plain chance left ~10%
  //    of them with zero incoming static links from anywhere on the site
  //    (Ahrefs flagged these as orphan pages). Rotating a shared window by
  //    REST_LINK_COUNT per page, keyed off currentSlug's stable rank in the
  //    sorted catalog, guarantees every long-tail game is picked up by many
  //    pages while still varying the set page to page.
  const REST_LINK_COUNT = 20;
  const allSlugs = Array.from(bySlug.keys()).sort();
  const rest = allSlugs.filter((s) => !used.has(s));
  if (rest.length) {
    const anchorIndex = Math.max(allSlugs.indexOf(currentSlug), 0);
    const start = (anchorIndex * REST_LINK_COUNT) % rest.length;
    for (let k = 0; k < Math.min(REST_LINK_COUNT, rest.length); k += 1) {
      push(rest[(start + k) % rest.length]);
    }
  }

  return ordered.slice(0, POPULAR_SLUGS.length + REST_LINK_COUNT).map((g) => {
